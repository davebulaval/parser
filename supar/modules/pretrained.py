...
class TransformerEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.max_len = (
            int(max(0, self.model.config.max_position_embeddings) or 1e12) - 2
        )
        self.stride = min(stride, self.max_len)

        self.scalar_mix = ScalarMix(self.n_layers, mix_dropout)
        self.projection = (
            nn.Linear(self.hidden_size, self.n_out, False)
...
class ScalarMix(nn.Module):
    ...
    def __init__(self, n_layers: int, dropout: float = 0.0) -> ScalarMix:
        super().__init__()
