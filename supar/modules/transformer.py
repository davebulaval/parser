...
class TransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        self.ffn = PositionwiseFeedForward(
            n_model=n_model, n_inner=n_inner, activation=activation, dropout=ffn_dropout
        )
...
class RotaryPositionTransformerEncoderLayer(TransformerEncoderLayer):
    def __init__(
        ...
        self.attn = RotaryPositionMultiHeadAttention(
            n_heads=n_heads,
            n_model=n_model,
            n_embed=n_model // n_heads,
            dropout=attn_dropout,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.pre_norm:
            ...
            x_tgt = x_tgt + self.dropout(
                self.self_attn(n_tgt, n_tgt, n_tgt, tgt_mask, attn_mask)
            )
            ...
            x_tgt = self.self_attn_norm(
                x_tgt
                + self.dropout(self.self_attn(x_tgt, x_tgt, x_tgt, tgt_mask, attn_mask))
            )
            x_tgt = self.mha_attn_norm(
                x_tgt + self.dropout(self.mha_attn(x_tgt, x_src, x_src, src_mask))
            )
...
class RelativePositionTransformerDecoderLayer(TransformerDecoderLayer):
    def __init__(
        ...
        self.self_attn_norm = nn.LayerNorm(n_model)
        self.mha_attn = RelativePositionMultiHeadAttention(
            n_heads=n_heads,
            n_model=n_model,
            n_embed=n_model // n_heads,
            dropout=attn_dropout,
...
class RotaryPositionTransformerDecoderLayer(TransformerDecoderLayer):
    def __init__(
        ...
        self.self_attn = RotaryPositionMultiHeadAttention(
            n_heads=n_heads,
            n_model=n_model,
            n_embed=n_model // n_heads,
            dropout=attn_dropout,
        ...
        self.self_attn_norm = nn.LayerNorm(n_model)
        self.mha_attn = RotaryPositionMultiHeadAttention(
            n_heads=n_heads,
            n_model=n_model,
            n_embed=n_model // n_heads,
            dropout=attn_dropout,
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        attn = torch.softmax(attn + torch.where(mask, 0.0, float("-inf")), -1)
        ...
        return (
            (x, attn.view(batch_size, self.n_heads, *attn.shape[1:]))
            if self.attn
            else x
        )
...
class RelativePositionMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        # [seq_len, batch_size * n_heads, n_embed]
        qu, qv = (q + self.bu).view(-1, *k.shape[1:]), (q + self.bv).view(
            -1, *k.shape[1:]
        )
        ...
        attn = attn + torch.matmul(
            qv.transpose(0, 1).unsqueeze(2), p.transpose(1, 2)
        ).squeeze(2)
        attn = torch.softmax(
            attn / self.scale + torch.where(mask, 0.0, float("-inf")), -1
        )
        attn = self.dropout(attn)
        # [seq_len, batch_size * n_heads, n_embed]
        x = torch.bmm(attn, v.transpose(0, 1)).transpose(0, 1)
...
class PositionalEmbedding(nn.Embedding):
    def __init__(self, n_model: int = 1024, max_len: int = 1024) -> PositionalEmbedding:
    ...
    def reset_parameters(self):
        ...
        w = w / 10000 ** (
            w.new_tensor(range(n_model)).div(2, rounding_mode="floor") * 2 / n_model
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return torch.embedding(
            self.weight, x.new_tensor(range(x.shape[1]), dtype=torch.long)
        )


class RelativePositionalEmbedding(nn.Module):
    def __init__(
        self, n_model: int = 1024, max_len: int = 1024
    ...
    def reset_parameters(self):
        ...
        max_len, n_model = w.shape
        pos = torch.cat(
            (w.new_tensor(range(-max_len // 2, 0)), w.new_tensor(range(max_len // 2)))
        )
        w = pos.unsqueeze(-1) / 10000 ** (
            w.new_tensor(range(n_model)).div(2, rounding_mode="floor") * 2 / n_model
        )
        w[:, 0::2], w[:, 1::2] = w[:, 0::2].sin(), w[:, 1::2].cos()
        self.weight.copy_(w)
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor) -> torch.Tensor:
        ...
        indices = (
            k.new_tensor(range(k.shape[0]))
            - q.new_tensor(range(q.shape[0])).unsqueeze(-1)
        ).long() + indices
...
class SinusoidPositionalEmbedding(nn.Module):
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        pos = pos / 10000 ** (
            x.new_tensor(range(n_model)).div(2, rounding_mode="floor") * 2 / n_model
        )
...
class RotaryPositionalEmbedding(nn.Embedding):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        pos = torch.embedding(
            self.weight, x.new_tensor(range(x.shape[0]), dtype=torch.long)
        ).unsqueeze(1)
