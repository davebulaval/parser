...
def ispunct(
    ...
    return (
        all(unicodedata.category(char).startswith("P") for char in token)
        if pos is None
        else pos in puncts
    )
...
def stripe(
    x: torch.Tensor, n: int, w: int, offset: Tuple = (0, 0), horizontal: bool = True
) -> torch.Tensor:
    r"""
    ...
    return x.as_strided(
        size=(n, w, *x.shape[2:]),
        stride=[(seq_len + 1) * numel, (1 if horizontal else seq_len) * numel]
        + stride[2:],
...
def expanded_stripe(
    x: torch.Tensor, n: int, w: int, offset: Tuple = (0, 0)
) -> torch.Tensor:
    r"""
...
def binarize(
    data: Union[List[str], Dict[str, Iterable]], fbin: str = None, merge: bool = False
    ...
    with open(fbin, "rb") as f, mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ) as mm:
        if meta or isinstance(pos_or_key, str):
            ...
            for offset, length in meta.tolist():
                mm.seek(offset)
                objs.append(
                    mm.read(length) if unpickle else pickle.loads(mm.read(length))
                )
            return objs
        # fetch by positions
...
def pad(
    ...
    size = [len(tensors)] + [
        max(tensor.size(i) for tensor in tensors) for i in range(len(tensors[0].size()))
    ]
...
@wait
def download(
    url: str, path: Optional[str] = None, reload: bool = False, clean: bool = False
) -> str:
