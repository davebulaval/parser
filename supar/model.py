...
class Model(nn.Module):
        ...
        **kwargs,
        ...
        if encoder == "lstm":
            self.word_embed = nn.Embedding(
                num_embeddings=self.args.n_words, embedding_dim=self.args.n_embed
            )
            ...
            if "lemma" in self.args.feat:
                self.lemma_embed = nn.Embedding(
                    num_embeddings=self.args.n_lemmas,
                    embedding_dim=self.args.n_feat_embed,
                )
                n_input += self.args.n_feat_embed
            if "elmo" in self.args.feat:
                self.elmo_embed = ELMoEmbedding(
            ...
            self.word_embed = TransformerWordEmbedding(
                n_vocab=self.args.n_words,
    ...
    def embed(self, words, feats=None):
        ...
        if isinstance(self.embed_dropout, IndependentDropout):
            ...
            embed = torch.cat(
                self.embed_dropout(word_embed, torch.cat(feat_embed, -1)), -1
            )
        else:
            embed = word_embed
            if len(feat_embed) > 0:
