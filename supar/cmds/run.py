...
def init(parser):
    ...
    parser.add_argument(
        "--seed", "-s", default=1, type=int, help="seed for generating random numbers"
    )
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="num of processes used for data loading",
    )
    parser.add_argument(
        "--cache", action="store_true", help="cache the data for fast loading"
    )
    parser.add_argument(
        "--binarize", action="store_true", help="binarize the data first"
    )
    parser.add_argument(
        "--amp", action="store_true", help="use automatic mixed precision for parsing"
    )
    parser.add_argument(
        "--dist",
        choices=["ddp", "fsdp"],
        default="ddp",
        help="distributed training types",
    )
    parser.add_argument(
        "--wandb", action="store_true", help="wandb for tracking experiments"
    )
...
def parse(local_rank, args):
    ...
    init_logger(
        logger, f"{args.path}.{args.mode}.log", "a" if args.get("checkpoint") else "w"
    )
    logger.info("\n" + str(args))
