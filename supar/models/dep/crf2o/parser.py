...
class CRF2oDependencyParser(BiaffineDependencyParser):
    ...
    def eval_step(self, batch: Batch) -> AttachmentMetric:
        ...
        mask[:, 0] = 0
        s_arc, s_sib, s_rel = self.model(words, feats)
        loss, s_arc, s_sib = self.model.loss(
            s_arc,
            s_sib,
            s_rel,
            arcs,
        ...
        arc_preds, rel_preds = self.model.decode(
            s_arc, s_sib, s_rel, mask, self.args.tree, self.args.mbr, self.args.proj
        )
    ...
    def pred_step(self, batch: Batch) -> Batch:
        ...
        s_arc, s_sib, s_rel = self.model(words, feats)
        s_arc, s_sib = (
            Dependency2oCRF((s_arc, s_sib), lens).marginals
            if self.args.mbr
            else (s_arc, s_sib)
        )
        arc_preds, rel_preds = self.model.decode(
    ...
    def build(cls, path, min_freq=2, fix_len=20, **kwargs):
        ...
        if os.path.exists(path) and not args.build:
            ...
            parser.model.load_pretrained(parser.transform.FORM[0].embed).to(
                parser.device
            )
            return parser
        ...
        if args.encoder == "bert":
            ...
            if "char" in args.feat:
                CHAR = SubwordField(
                    "chars", pad=PAD, unk=UNK, bos=BOS, fix_len=args.fix_len
                )
        ...
        transform = CoNLL(
            FORM=(WORD, TEXT, CHAR, ELMO, BERT), CPOS=TAG, HEAD=(ARC, SIB), DEPREL=REL
        )
