...
class TetraTaggingConstituencyParser(Parser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.left_mask = torch.tensor(
            [
                *(i.startswith("l") for i in self.LEAF.vocab.itos),
                *(i.startswith("L") for i in self.NODE.vocab.itos),
            ]
        ).to(self.device)
    ...
    def eval_step(self, batch: Batch) -> SpanMetric:
        ...
        preds = [
            TetraTaggingTree.action2tree(
                tree, (self.LEAF.vocab[i], self.NODE.vocab[j] if len(j) > 0 else [])
            )
        ...
        return SpanMetric(
            loss,
            [
                TetraTaggingTree.factorize(tree, self.args.delete, self.args.equal)
                for tree in preds
            ],
            [
                TetraTaggingTree.factorize(tree, self.args.delete, self.args.equal)
    ...
    def pred_step(self, batch: Batch) -> Batch:
        ...
        if self.args.prob:
            raise NotImplementedError(
                "Returning action probs are currently not supported yet."
            )
        return batch
    ...
    def build(cls, path, min_freq=2, fix_len=20, **kwargs):
        ...
        transform = TetraTaggingTree(
            WORD=(WORD, CHAR, ELMO, BERT), POS=TAG, TREE=TREE, LEAF=LEAF, NODE=NODE
        )
