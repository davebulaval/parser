...
class AttachJuxtaposeConstituencyParser(Parser):
    ...
    def eval_step(self, batch: Batch) -> SpanMetric:
        ...
        preds = [
            AttachJuxtaposeTree.build(
                tree,
                [(i, j, self.NEW.vocab[label]) for i, j, label in chart],
                {UNK, NUL},
            )
        ...
        return SpanMetric(
            loss,
            [
                AttachJuxtaposeTree.factorize(tree, self.args.delete, self.args.equal)
                for tree in preds
            ],
            [
                AttachJuxtaposeTree.factorize(tree, self.args.delete, self.args.equal)
    ...
    def build(cls, path, min_freq=2, fix_len=20, **kwargs):
        ...
        TREE = RawField("trees")
        NODE, PARENT, NEW = (
            Field("node", use_vocab=False),
            Field("parent", unk=UNK),
            Field("new", unk=UNK),
        )
        transform = AttachJuxtaposeTree(
            WORD=(WORD, CHAR, ELMO, BERT),
            POS=TAG,
            TREE=TREE,
            NODE=NODE,
            PARENT=PARENT,
