...
class DependencyMFVI(nn.Module):
    ...
    def mfvi(self, s_arc, s_sib, mask):
        ...
        ls, rs = (
            torch.stack(torch.where(mask.new_ones(seq_len, seq_len)))
        ...
        mask2o = mask2o & ls.unsqueeze(-1).ne(ls.new_tensor(range(seq_len))).unsqueeze(
            -1
        )
        mask2o = mask2o & rs.unsqueeze(-1).ne(rs.new_tensor(range(seq_len))).unsqueeze(
            -1
        )
        # [seq_len, seq_len, batch_size], (h->m)
...
class DependencyLBP(nn.Module):
    ...
    def lbp(self, s_arc, s_sib, mask):
        ...
        for _ in range(self.max_iter):
            ...
            m_sib = (
                torch.logaddexp(m.logsumexp(0), m + s_sib)
                .transpose(1, 2)
                .log_softmax(0)
            )
...
class ConstituencyMFVI(nn.Module):
    ...
    def mfvi(self, s_span, s_pair, mask):
        ...
        ls, rs = (
            torch.stack(torch.where(torch.ones_like(mask[0])))
            .view(-1, seq_len, seq_len)
            .sort(0)[0]
        )
...
class ConstituencyLBP(nn.Module):
    ...
    def lbp(self, s_span, s_pair, mask):
        ...
        for _ in range(self.max_iter):
            ...
            m_pair = torch.stack(
                (m.logsumexp(0), torch.stack((m[0], m[1] + s_pair)).logsumexp(0))
            ).log_softmax(0)
            # q(ij) = s(ij) + sum(m(ik->ij)), k != i,j
...
class SemanticDependencyMFVI(nn.Module):
    ...
    def mfvi(self, s_edge, s_sib, s_cop, s_grd, mask):
        ...
        # [seq_len, seq_len, seq_len, batch_size], (h->m->s)
        mask2o = mask.unsqueeze(1) & mask.unsqueeze(2)
        mask2o = mask2o & hs.unsqueeze(-1).ne(hs.new_tensor(range(seq_len))).unsqueeze(
            -1
        )
        mask2o = mask2o & ms.unsqueeze(-1).ne(ms.new_tensor(range(seq_len))).unsqueeze(
            -1
        )
    ...
    def lbp(self, s_edge, s_sib, s_cop, s_grd, mask):
        ...
        for _ in range(self.max_iter):
            ...
            m_sib = torch.stack(
                (
                    v_sib.logsumexp(0),
                    torch.stack((v_sib[0], v_sib[1] + s_sib)).logsumexp(0),
                )
            ).log_softmax(0)
            # coparent factor
            ...
            m_cop = torch.stack(
                (
                    v_cop.logsumexp(0),
                    torch.stack((v_cop[0], v_cop[1] + s_cop)).logsumexp(0),
                )
            ).log_softmax(0)
            # grandparent factor
            ...
            m_grd = torch.stack(
                (
                    v_grd.logsumexp(0),
                    torch.stack((v_grd[0], v_grd[1] + s_grd)).logsumexp(0),
                )
            ).log_softmax(0)
            # tree factor
            v_tree = q - m_tree
            b_tree = DependencyCRF(
                (v_tree[1] - v_tree[0]).permute(2, 1, 0), lens
            ).marginals.permute(2, 1, 0)
