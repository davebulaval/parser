...
class TestCoNLL:
    ...
    def test_istree(self):
        permutations = [
            list(sequence[:5]) for sequence in itertools.permutations(range(6))
        ]
        for sequence in permutations:
            assert CoNLL.istree(sequence, False, False) == self.istree_naive(
                sequence, False, False
            ), f"{sequence}"
            assert CoNLL.istree(sequence, False, True) == self.istree_naive(
                sequence, False, True
            ), f"{sequence}"
            assert CoNLL.istree(sequence, True, False) == self.istree_naive(
                sequence, True, False
            ), f"{sequence}"
            assert CoNLL.istree(sequence, True, True) == self.istree_naive(
                sequence, True, True
            ), f"{sequence}"
