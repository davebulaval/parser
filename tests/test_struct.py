...
class BruteForceStructuredDistribution(Distribution):
    def __init__(self, scores, **kwargs):
        ...
        self.scores = (
            ...
            else [s.requires_grad_() for s in scores]
        )
    ...
    def log_partition(self):
        return torch.stack(
            [LogSemiring.sum(i, -1) for i in self.enumerate(LogSemiring)]
        )

    @lazy_property
    def max(self):
        return torch.stack(
            [MaxSemiring.sum(i, -1) for i in self.enumerate(MaxSemiring)]
        )
    ...
    def entropy(self):
        ps = [
            seq - self.log_partition[i]
            for i, seq in enumerate(self.enumerate(LogSemiring))
        ]
    ...
    def cross_entropy(self, other):
        ...
        qs = [
            seq - other.log_partition[i]
            for i, seq in enumerate(other.enumerate(LogSemiring))
        ]
        ...
        for i, length in enumerate(self.lens.tolist()):
            ...
            for seq in itertools.product(range(length + 1), repeat=length):
                if not CoNLL.istree(list(seq), True, self.multiroot):
                    continue
                trees[-1].append(
                    semiring.prod(self.scores[i, range(1, length + 1), seq], -1)
                )
    ...
    def enumerate(self, semiring):
        ...
        for i, length in enumerate(self.lens.tolist()):
            ...
            for seq in itertools.product(range(length + 1), repeat=length):
                ...
                s_sib = semiring.prod(
                    s_sib[1:][sib_mask]
                    .gather(-1, sibs[sib_mask].unsqueeze(-1))
                    .squeeze(-1)
                )
...
class BruteForceConstituencyCRF(BruteForceStructuredDistribution):
    ...
    def enumerate(self, semiring):
        ...
        return [
            torch.stack([i for i in enumerate(s, 0, length)])
            for s, length in zip(scores, self.lens)
        ]
...
class BruteForceLinearChainCRF(BruteForceStructuredDistribution):
    def __init__(self, scores, trans=None, lens=None):
        ...
        self.lens = (
            scores.new_full((batch_size,), seq_len).long() if lens is None else lens
        )
        self.mask = self.lens.unsqueeze(-1).gt(self.lens.new_tensor(range(seq_len)))

        self.trans = (
            self.scores.new_full((self.n_tags + 1, self.n_tags + 1), LogSemiring.one)
            if trans is None
            else trans
        )
    ...
    def enumerate(self, semiring):
        ...
        for i, length in enumerate(self.lens.tolist()):
            ...
            for seq in itertools.product(
                range(self.scores[0].shape[-1]), repeat=length
            ):
                prev, succ = (-1,) + seq, seq + (-1,)
                seqs[-1].append(
                    semiring.prod(
                        torch.cat(
                            (self.scores[i, range(length), seq], self.trans[prev, succ])
                        ),
                        -1,
                    )
                )
...
class BruteForceSemiMarkovCRF(BruteForceStructuredDistribution):
    def __init__(self, scores, trans=None, lens=None):
        ...
        self.trans = (
            self.scores.new_full((self.n_tags, self.n_tags), LogSemiring.one)
            if trans is None
            else trans
        )
        ...
        for i, length in enumerate(self.lens.tolist()):
            ...
            for seg in self.segment(length):
                ...
                for t in itertools.product(range(self.n_tags), repeat=len(seg)):
                    seqs[-1].append(
                        semiring.prod(
                            torch.cat((scores[l, r, t], self.trans[t[:-1], t[1:]])), -1
                        )
                    )
        return [torch.stack(seq) for seq in seqs]
    ...
    def segment(cls, length):
        if length == 1:
            return [[(0, 0)]]
        return [
            s + [(i, length - 1)] for i in range(1, length) for s in cls.segment(i)
        ] + [[(0, length - 1)]]
...
def test_struct():
    ...
    def enumerate():
        ...
        yield (
            DependencyCRF(s1, lens),
            DependencyCRF(s2, lens),
            BruteForceDependencyCRF(s1, lens),
            BruteForceDependencyCRF(s2, lens),
        )
        yield (
            DependencyCRF(s1, lens, multiroot=True),
            DependencyCRF(s2, lens, multiroot=True),
            BruteForceDependencyCRF(s1, lens, multiroot=True),
            BruteForceDependencyCRF(s2, lens, multiroot=True),
        )
        s1 = [
            torch.randn(batch_size, seq_len, seq_len).to(device),
        ...
        yield (
            Dependency2oCRF(s1, lens),
            Dependency2oCRF(s2, lens),
            BruteForceDependency2oCRF(s1, lens),
            BruteForceDependency2oCRF(s2, lens),
        )
        yield (
            Dependency2oCRF(s1, lens, multiroot=True),
            Dependency2oCRF(s2, lens, multiroot=True),
            BruteForceDependency2oCRF(s1, lens, multiroot=True),
            BruteForceDependency2oCRF(s2, lens, multiroot=True),
        )
        s1 = torch.randn(batch_size, seq_len, seq_len, n_tags).to(device)
        s2 = torch.randn(batch_size, seq_len, seq_len, n_tags).to(device)
        yield (
            ConstituencyCRF(s1, lens, True),
            ConstituencyCRF(s2, lens, True),
            BruteForceConstituencyCRF(s1, lens, True),
            BruteForceConstituencyCRF(s2, lens, True),
        )
        ...
        yield (
            LinearChainCRF(s1, lens=lens),
            LinearChainCRF(s2, lens=lens),
            BruteForceLinearChainCRF(s1, lens=lens),
            BruteForceLinearChainCRF(s2, lens=lens),
        )
        yield (
            LinearChainCRF(s1, t1, lens=lens),
            LinearChainCRF(s2, t2, lens=lens),
            BruteForceLinearChainCRF(s1, t1, lens=lens),
            BruteForceLinearChainCRF(s2, t2, lens=lens),
        )
        s1 = torch.randn(batch_size, seq_len, seq_len, n_tags).to(device)
        s2 = torch.randn(batch_size, seq_len, seq_len, n_tags).to(device)
        ...
        yield (
            SemiMarkovCRF(s1, lens=lens),
            SemiMarkovCRF(s2, lens=lens),
            BruteForceSemiMarkovCRF(s1, lens=lens),
            BruteForceSemiMarkovCRF(s2, lens=lens),
        )
        yield (
            SemiMarkovCRF(s1, t1, lens=lens),
            SemiMarkovCRF(s2, t2, lens=lens),
            BruteForceSemiMarkovCRF(s1, t1, lens=lens),
            BruteForceSemiMarkovCRF(s2, t2, lens=lens),
        )
